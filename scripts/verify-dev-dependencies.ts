...
const pkg = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8')) as typeof import('../package.json')
for (const [name, version] of Object.entries(pkg.devDependencies)) {
  ...
  const manifest = JSON.parse(readFileSync(join(directory, 'package.json'), 'utf8')) as typeof import('../package.json')
