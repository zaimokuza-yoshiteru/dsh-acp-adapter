...
import { PLATFORM_EXTERNALS, PLATFORM_PACKAGES } from './dsh-platform-snapshot.ts'
import { findMissingRelativeRuntimeImports } from './verify-runtime-closure.ts'
import { DSH_COMPAT_RANGE, DSH_SOURCE_VERSION } from './dsh-target.ts'
...
function checkStringArray(field: string, value: unknown, { required = false } = {}) {
  if (value === undefined) {
    if (required) fail(`package.json: ${field} 缺失（必须完整声明包名依赖边）`)
    return []
  }
  if (!Array.isArray(value) || value.some((item: unknown): item is string => typeof item !== 'string')) {
...
const REQUIRED_ARTIFACTS = [
  ...
  // typert Remote 生成物（scripts/gen-typert.ts；.d.ts.map 不进 files 是故意的）
  'lib/typert.host.js',
...
for (const rel of globSync('lib/**/*', { cwd: root })) {
...
const serviceReads = new Set<string>()
...
if (packOutput !== null) {
  ...
  if (tar !== null) {
    const actual = new Set<string>((tar.files ?? []).map((file: { path: string }) => file.path.replaceAll('\\', '/')))
