 ...
 */
export function findMissingRelativeRuntimeImports(files: readonly string[], readFile: (file: string) => string) {
...
function resolveRuntimeImport(file: string, specifier: string) {
  const base = normalize(join(dirname(file), specifier)).replaceAll('\\', '/')
  if (base === '..' || base.startsWith('../')) return undefined
  return base
}

function runtimeCandidates(resolved: string) {
