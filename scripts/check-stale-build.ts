...
// 打包太慢；由维护者在发布前通过 `pnpm check:stale-build` 单独运行）。
//
...
function pnpm(args: string[], options: import('node:child_process').ExecFileSyncOptions = {}) {
  const opts = { cwd: root, stdio: 'inherit' as const, shell: process.platform === 'win32', ...options }
...
function packFileList() {
  ...
  try {
    ...
    return new Set<string>((tar?.files ?? []).map((file: { path: string }) => file.path))
  } finally {
