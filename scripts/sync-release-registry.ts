...
import type { RegistrySnapshot, ExecutableSnapshot, ExecutableEntry } from './registry-snapshot.ts'
import { validateSnapshot } from './registry-snapshot.ts'


export interface RegistrySyncReport {
  status: 'fallback' | 'updated' | 'unchanged'; source: string; sourceCommit: string; checkedAt: string;
  baselineHashes: Record<string, string>; hashes: Record<string, string>; agentCount: number;
  changes: ReturnType<typeof changes>; error?: string;
}
...
const hashes = (directory: string) => Object.fromEntries(files.map(name => [name, createHash('sha256').update(readFileSync(join(directory, name))).digest('hex')]))
const snapshot = (directory: string) => {
  const registry = read<RegistrySnapshot>(join(directory, files[0]))
  const executables = read<ExecutableSnapshot>(join(directory, files[1]))
  validateSnapshot(registry, executables)
  return { registry, executables }
}
const escape = (value: unknown) => String(value).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('@', '&#64;').replaceAll('`', '&#96;').replaceAll('\r', '').replaceAll('\n', ' ')

function changes(before: ReturnType<typeof snapshot>, after: ReturnType<typeof snapshot>) {
  ...
  const launch = (entry: ExecutableEntry) => Object.fromEntries(Object.entries(entry).filter(([key]) => key !== 'version'))
  return {
...
function summary(report: RegistrySyncReport) {
...
if (process.argv.includes('--reuse') && existsSync(join(evidence, 'report.json'))) {
  report = read<RegistrySyncReport>(join(evidence, 'report.json'))
  snapshot(evidence)
  ...
  try {
    ...
    execFileSync(process.execPath, [...process.execArgv, join(scripts, 'enrich-registry-executables.ts'), '--file', join(stage, files[0]), '--out', join(stage, files[1])], {
      encoding: 'utf8', stdio: 'pipe', timeout: 300_000, maxBuffer: 1024 * 1024,
    })
    next = snapshot(stage)
    selected = stage
  } catch (error) {
    failure = String((error instanceof Error ? error.message : String(error)) ?? error).slice(0, 6000)
  }
  try {
