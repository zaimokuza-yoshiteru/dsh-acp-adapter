# 2026-09-24 DSH 0.1.7-rc.1 迁移

目标为正式 npm `0.1.7-rc.1` 与源码标签 `dsh-v0.1.7-rc.1`；候选分支 `codex/dsh-0.1.7-rc.1-compat`。Luna 分别实施依赖、Teams 和活动流修复，主审查者复核后集成。未修改插件发布版本，未发布 npm 或更新桌面内置 pin。

依赖声明和安装准入对齐 RC，运行中工具补齐原生 `phase`；Teams 展示迁移到共享投影，ACP 元数据保留归属检查并按状态变化刷新；活动流使用单个可清理的取消监听器与修订高水位。复核修正了跨团队待审批误显示、当前／待生效模型混淆和空闲后控件状态不刷新等实现问题。

| 验证 | 实际结果 |
| --- | --- |
| 类型检查、构建、声明消费方、导入闭包、包内容与 npm RC 安装启动 | 通过；81 个运行时 JS 文件、215 个打包文件；隔离安装、卸载、HTTP 与客户端启动通过 |
| 单元与集成测试 | 新增团队投影隔离用例后，Linux、macOS、Windows 均为 94 文件、926 项通过 |
| 首轮完整 Web 协议回归 | 138 通过、6 失败、8 跳过；失败均为旧 UI 断言：已移除的团队刷新按钮、无成员时的管理入口、旧工具摘要拼接文本 |
| 修正后完整本地 Web 回归 | 144 通过、8 跳过、0 失败；跳过项为显式启用的真实 Agent 测试与不适用的协议分支 |
| RC 流分段专项 | 四种 ACP 夹具通过四种模式、旧设置别名、正常完成、取消、刷新恢复和顺序／去重检查；verbose 模式不再要求隐藏的分组标题可点击 |
| Teams 定向复测 | 原生任务板实时更新通过；新增成员面板保持打开的运行→空闲场景通过，验证当前／待生效模型区别及模型菜单重新可写 |
| Electron 44 隔离窗口 | 会话控件、成员管理、团队卡片共 3 文件、10 项通过；不代替 Windows 桌面安装包或自动升级测试 |
| 真实 Devin 3000.10.31 | [macOS / Windows CI](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter/actions/runs/35948338440) 均通过 |
| 真实 Devin 3000.3.27 | [macOS / Windows CI](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter/actions/runs/35948880640) 均通过 |

真实测试在每个平台启动两支独立团队、同名 checker 成员，通过真实 `spawn_teammate` 和 `send_message`，检查消息在正确 Lead 会话中的消费记录及其后回复，再在团队空闲后提交独立续聊并验证该轮完成。成员回复可在 Lead 同一轮的下一步消费，因此不再把固定“两轮”计数当作消息送达条件。Windows 产品步骤由独立普通账号执行。用户明确授权取消 `devin-e2e` 的部署分支限制；现有 CI 凭据未迁出，手动运行可选择仓库任意分支，自动触发分支范围保持原样。

Linux CI 随后发现两项模式切换断言的时序竞争：宿主设置写入返回时，浏览器尚未应用推送；测试先取得旧分组按钮，进入 verbose 后该定位器便不再匹配。测试现在先等待原生折叠控件与分组状态共同确认新展示策略，再枚举分组，保留所有内容顺序与去重断言。最终 CI 结果见对应 PR。

上游 RC API account controller 仍显式依赖 alpha.1 的 account 包，而其他 RC 包需要其 RC peer。锁文件保留上游依赖关系，干净安装、跨平台类型／构建及宿主启动均通过；未添加直接 account 依赖掩盖此拓扑。成功 assistant/message 到 step/end 之间的瞬态保留由 RC 原生测试覆盖；本插件的完整流回归覆盖最终收敛和取消，尚无单独暂停该内部时间窗口的浏览器断言。

# 2026-09-23 整体 ACP 实现审查

宿主：正式 npm 与源码标签 `0.1.7-alpha.2`；分支 `feature/ZAI-88`。本轮检查覆盖执行与恢复、协议生命周期、输入与配置时序、审批、DSH 工具桥、文件/终端、Sidecar、主会话归一化展示、配置与分发。

修复：DSH 工具名保留原生名称、由连接绑定调用方；无界面宿主使用原生请求头完成连续性校验；子会话通知不能覆盖主会话配置/模式/用量；配置写入先于后续 prompt 完成，关闭不复活进程；首次活动流失败不再当成空记录；Devin 注册后核验实际生效入口。工具调用继续经过原生 tools.execute 与插件 hooks，普通操作的审批范围没有扩大。

| 验证 | 本轮实际结果 |
| --- | --- |
| 类型检查、构建、导入闭包、声明消费方、打包文件校验 | 通过；81 个运行时 JS 文件、214 个打包文件 |
| 单元/集成测试 | 91 文件、915 项通过；外部子会话污染、配置写入竞争、活动流首次失败的新增反例先在原代码失败，再在修复后通过 |
| 全量 Web 协议回归首轮 | 141 通过、3 失败、8 跳过；3 项 Teams 失败来自测试夹具查找旧前缀 bash 名称 |
| 最终构建 Web 受影响部分 | Teams 与流分段通过；原生展示复测共 108 通过、1 失败、3 跳过。唯一失败为异步模型菜单恢复焦点与测试输入竞争，等待当前模型及菜单关闭后，四种协议的模型切换全部通过；不放宽实际模型 B 请求断言 |
| Electron 44 隔离窗口 | 会话控件与团队卡片 2 文件、9 项通过；不代表桌面安装包、自动升级或 Windows 成品验证 |
| 真实 Devin 3000.3.27 / macOS | 最终构建通过：两支独立团队，各有同名 checker，真实 spawn_teammate / send_message 成功，回复分别送达自己的 Lead，两个 Lead 第二轮完成；测试退出清理临时配置，没有人工批准协调请求 |

协议回归使用四种 Agent 夹具，不把其结果表述为四家真实服务验证。模型选择失败证据是当前模型已变成 B、输入框为空；上游 ModelSelect 在异步选择完成时关闭菜单并恢复触发器焦点。测试现在以这项完成状态作为后续输入边界。用户在选择未完成时快速输入是否也受影响，尚未单独进行人工交互验证，本轮未改上游组件。

Windows 的真实 Devin 回归未执行：受保护的 `devin-e2e` GitHub 环境未授权 `feature/ZAI-88` 使用 secrets。本轮没有更改该环境保护规则，没有上传本机凭证，也未将本机测试当成跨平台结论。详细实现边界见中英文 [原生复用说明](../../docs/native-reuse.md)。

# 历史回归记录

本页保留历史验收快照，不表示当前版本的验证状态。复现步骤与验收矩阵统一维护在 [E2E 指南](README.md)；后续逐次结果记录在对应 PR／CI，避免重复维护测试数量、本机路径和发布状态。

`0.1.6-alpha.1.4` 的变更见 [PR #6](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter/pull/6)，自动检查见 [PR CI](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter/actions/runs/35117367723)，发布状态见 [发布工作流](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter/actions/runs/35119168694)。该次本机验收明细可在 [当时提交的记录](https://github.com/zaimokuza-yoshiteru/dsh-acp-adapter/blob/ce1893edd49dfc333cb8b0bc64d566308b41c39b/test/e2e/REGRESSION.md) 中追溯；其中候选包和未发布状态属于当时快照。真实模型、本机 Electron 与 CI 协议夹具结果不可相互替代。

---

# 2026-09-16 消息顺序与成员卡片

宿主：正式 `dsh-v0.1.6-alpha.1`；本地插件候选，未修改版本或发布。浏览器使用 Chrome 与 Claude、Codex、Devin、Kimi 四类本地 ACP 协议夹具，不调用真实模型。

- 活动首次出现的位置与原生内容共用串行队列；Sidecar 保存内容边界，后续状态或详情更新不移动原调用。异步图片、工具详情替换与取消收敛均有覆盖，不生成 DSH 工具执行事件。
- 使用公开插槽组合现有原生 assistant renderer，保留 Markdown、思考折叠、附件和文件跳转。活动等待前置内容到达，取消后的截断内容仍保留工具记录；原生会话直接委托原 renderer。第三方 renderer 有私有依赖、子插槽或已占用优先级时，保留独立活动展示。
- 旧数据库自动添加可空 `content_index` 列。旧记录缺少可靠位置，继续原分组展示，不推断或重写历史，无需清理本地数据。
- 成员卡片至少 220px，说明最多三行，设置区靠底对齐；提示采用固定 13px 行高、10px 字号。长名称、双语、明暗主题、窄窗口及修改前后卡片尺寸均验证。

| 检查 | 结果 |
| --- | --- |
| 类型、构建、严格公共类型消费方 | 通过 |
| 最终单元与契约测试 | 72 文件、726 项通过 |
| 四 ACP 原生功能对照 | 64 项全部通过，含上传、图片/Read/Diff、恢复、停止、审批、模型、队列、子代理和原生 provider 对照 |
| 最终受影响的浏览器复测 | 13 项全部通过：4 种 ACP 的思考/正文/工具顺序、实时与取消/刷新恢复，8 项附件与工具详情，1 项双语卡片布局 |
| 全量运行与复测汇总 | 110 个确定性场景中，109 个取得通过；原生终端清理仍不稳定。4 项真实服务冒烟未运行；不是同一次全量全绿 |
| 上游 reference、差异空白检查 | reference 干净，`git diff --check` 通过 |

首轮全量发现新内联活动区遗漏 `data-acp-activity` 标记，已补回并复测详情展开、图片和文件跳转。新增取消测试曾使用错误大小写的按钮标签，且写草稿早于新会话完成切换；现等待旧会话退出后输入。位置断言改为同一浏览器帧读取全部坐标，避免原生自动滚动造成跨帧假倒序；仍检查严格视觉顺序和每个工具只显示一次。

未解决项：`browser-terminal-access` 在两次组合运行中报 `LocalTerminalHandle.closeOnce: terminal cleanup failed; surviving pids`，独立复测曾通过；与此前桌面回归记录中的上游清理时序问题一致。最终只读检查这些 PID 均已退出，没有持续残留。本轮未修改宿主的进程树终止实现，也未以重试掩盖该失败。

实际页面截图保存在 `.local/message-order-review/`：`stream.live.png`、`stream.en.png`、`members.zh.png`、`members.en.png`。测试实例关闭后截图保留，未操作用户原有 Chrome 窗口。

# 2026-09-15 macOS 桌面成品回归

本轮在独立 DSH_HOME 中运行实际打包的 macOS arm64 Electron 应用。核心来自桌面仓库当前构建，插件来自本地 tarball；未改生产插件版本 pin、未发布 npm。测试使用 Devin、Kimi、Codex、Claude 四种本地 ACP 协议夹具，不调用真实模型服务。

本轮修复：

- 桌面打包显式包含 `plugin-seed/node_modules`，避免 Electron Builder 过滤嵌套依赖后首启找不到内置插件。
- ACP 中断回答的历史恢复不再要求 preceding request 一定已经装配；只复用同一提示词且未结束的活动，避免刷新后整段会话空白。
- 既有 Teams 的 Lead 尚未激活时，通过宿主 `sessionController.resolveAgent()` 恢复精确归属；成员面板不再因冷启动消失，不提交额外提示词。
- 桌面主动关闭或获准的空闲重启不再触发“后端崩溃”回调，避免切换 Teams 后停在启动页；真实异常退出仍通知。
- 源码 E2E 先安装所选宿主的 Teams layers，避免误用插件开发依赖中的旧版 Teams 元数据。
- 成品检查验证实际安装的 Host/Remote 元数据以及 ACP 和 Plugin Hub RPC。原 npm 插件缺少 `create()` 的负向样本仍被拒绝，没有桌面格式兼容补丁。

| 验证范围 | 结果 |
| --- | --- |
| 插件类型、构建、严格公共类型消费方 | 通过 |
| 插件全量单元与契约测试 | 71 文件、722 项通过 |
| 桌面与 desktop-host 测试 | 41 文件、230 项通过、1 项平台相关跳过 |
| 正式 DSH alpha.1 完整浏览器回归 | 109 项通过；4 项真实模型测试跳过 |
| 最新桌面宿主完整浏览器回归 | 两个隔离 worker：108 项通过、1 项原生终端清理失败、4 项真实模型跳过；失败项单独重跑通过 |
| 实际桌面交互 | 20 个场景通过：启动及插件激活、RPC、工作区、四 Agent 新会话、选项、允许/拒绝、取消后继续、模型切换、历史分段、图片/工具、Jobs、子代理、共享任务、集中审批、成员模型/模式、双语、Teams 开关重启 |
| 最终 .app 离线 smoke | 内置插件文件、Host/Remote 注册、ACP/Hub 请求、冷/暖启动、Teams 开关、正常/截断请求管道 EOF 通过 |
| 桌面静态检查、中英文文档检查 | lint 通过；快速文档 20 项、完整文档 41 项通过 |

仍需关注：最新宿主的 `browser-terminal-access` 在并行全量运行时出现一次 `terminal cleanup failed; surviving pids`，栈位于上游 `subprocess-local`。事后检查所列 PID 均已退出，单项重跑 5.75 秒通过。关闭路径在终止 shell 前扫描后代进程，可能与交互 shell 启动时继续派生进程存在时序竞争；这是待进一步确认的推断，本轮未修改上游进程树清理算法，也不把复测通过表述为首次全量无失败。

实际桌面的冷启动沿用已有测试会话，模式、模型偏好和历史均保留，无需清理本地数据。协议测试覆盖不等于四家真实服务、登录和企业 MCP 策略均已验证；Windows 实机、macOS 公证和原生文件选择器手动操作不在本轮已执行范围。

当前生产 pin 仍指向旧的已发布 ACP adapter；本地候选与 npm 同名同版本不代表内容一致。正式桌面分发前必须发布包含修复的新插件版本、更新 pin/integrity 并重跑成品检查，不能直接把本地候选通过宣称为现有 npm 组合可发布。

# 2026-09-15 RPC 元数据兼容修复

问题：上游 `e459e3263733075bd806d9ba6dd92bbc4bf3983f` 将严格 codec 从 `schema` 改为 `create()`，但源码版本仍为 `0.1.6-alpha.1`。已发布 adapter 的旧生成产物无法通过新 Loader 校验；不是桌面 UI 专属问题。

插件生成阶段为 Host 和 Remote 严格 codec 同时提供 `schema` 与 `create()`，两者共用原始校验器。该项修复不依赖桌面兼容补丁，不改变版本约束或本地数据格式。

| 验证 | 结果 |
| --- | --- |
| 类型检查、全量单元/契约测试 | 通过；71 个文件、717 项测试 |
| 构建与包检查 | 通过；177 个文件、64 个运行时 JS 依赖闭包、严格类型消费方 |
| 正式 alpha.1 Loader / Registry / Gateway | 新产物加载、Host 与 Remote 各 19 个 RPC 注册、客户端至宿主调用通过；两端拒绝非法输入 |
| 上游 `0d1f50007f9bca3f52b06e1c3074fa14d5fb0720` 同一链路 | 从 Git 对象提取未经本地补丁修改的 Loader、Registry、Host/Client Gateway 源码，结果同上；依赖使用已安装的 alpha.1 包 |
| 负向对照 | 已发布 `0.1.6-alpha.1.1` 的原始 npm 产物被新 Loader 拒绝，明确报缺少 `create()` |

这一阶段仅验证 RPC 兼容路径；后续完整桌面验证见本文件开头的成品回归记录。未对该上游提交的全部功能作兼容性承诺，没有重新调用真实模型，也未发布 npm 版本。
...
## 2026-09-14：目录恢复与主会话集中审批

模型探测失败交给原生目录显示为 provider 失败；显式重新检查结束后通过原生 adapter registration 通知目录刷新，不再依赖切换模型触发设置更新。检查不改写原会话历史，也不解除执行结果未知时的恢复保护。

主会话待处理卡复用原生 pending approval 的回答接口，提供逐项和全部允许／拒绝。批量只处理点击时的当前成员请求，允许仅限本次；重新验证团队归属、请求对象和当前会话，隔离部分失败及重复点击。新增请求不加入先前批次。通用提问不参与批量操作；宿主拒绝受其他存活 Agent 管理的子会话调用 `userQuestions.ask()`，因此未加入另一套表单 UI。

- 类型检查、64 个文件 671 项常规测试、构建、严格消费方类型与打包闭包检查通过。
- Chrome 完整固定夹具回归：99 项通过，2 项新增审批测试因把完整命令说明当成独立文本定位而超时；修正定位后两项复测通过，最终覆盖 101 项。4 项真实模型测试未启用，未直接验证桌面安装包。
- 新增四种协议的断网／检查失败后目录恢复测试，验证原会话历史不变且可继续对话；新增八成员逐项与批量允许／拒绝、刷新恢复、中英文、窄屏及后来请求仍待处理的测试。独立测试实例将原生团队上限设为 9（含 Lead），用户默认配置不变。
- 中英文 README 已同步；存储格式未改，无须清理旧数据。成员会话统一管理仅提供方案；本轮未提交或发布。

本轮检查日志和成功截图位于 gitignored `.local/team-approvals/`。

## 2026-09-14：主会话成员管理预览

输入框上方新增可折叠成员管理，复用原生按钮、菜单与现有 Agent 选项控件。主会话可查看成员状态、成员自身的模型及最后报告的上下文信息，发送消息和中断通过原生 Teams 信箱／中断接口完成。后端同时校验 Lead、真实成员归属与 ACP profile，不把原生 roster 的执行 backend 当成模型 provider。

- DSH 会释放完成的成员；此时仅展示最后报告的选项，不能修改，发送消息沿原生路径唤醒。仍存活且空闲的成员使用原有会话选项写入入口，不新增模型切换或重建会话能力。
- 类型检查、构建、65 个文件 674 项常规测试通过；Chrome 的成员管理、集中审批和 Teams 边界三套共 6 项通过。验证中断取消审批、主会话发消息、休眠后再次唤醒、刷新恢复、只读保护、跨会话拒绝、中英文和窄屏。此次未重新运行全部 102 项浏览器用例，也未调用真实模型。
- 演示使用独立本地 ACP 夹具，准备两个成员、两项有依赖的原生任务和成员审批。存储格式未改，无须清理旧数据；未提交或发布。
- 最终布局调整后，成员管理与集中审批共 3 项 Chrome 用例再次通过；新增短窗口中两张卡片同时展开、成员标题不被固定页签栏遮挡的断言。

演示日志及截图位于 gitignored `.local/team-management/`。

### 成员入口与审批排版调整

移除输入框上方的成员管理，改在会话右上角使用人员组图标打开弹层；沿用原生浮层定位、外部点击关闭、状态点、按钮与菜单，以及 Agent Team 的面板／卡片主题变量。成员卡片默认仅显示摘要，右下角「会话管理」展开原有操作。审批批量按钮移到卡片标题右侧，单项操作放到成员行右侧，详情在下方；说明按原生 ApprovalPanel headline 使用界面字体、15px 字号和 24px 行高，保留换行，不再整段使用等宽字体。

- 674 项常规测试、类型检查与构建通过；相关 Chrome E2E 覆盖的 6 项全部通过（成员用例修复 Esc 关闭后单独复测通过）。补充顶部入口位置、成员卡片默认收起、弹层边界、Esc 关闭、审批按钮排列和字体的浏览器断言。未调用真实模型、未提交或发布。

### 成员管理精简为 Agent 模式

成员卡片直接使用原生 Menu 展示 Agent 模式；移除操作展开区、消息输入、中断按钮和相应未发布的 Remote 接口。按成员真实 ACP profile 分组，每组提供独立批量模式菜单。仅支持模式设置，不包含模型、推理或其他 Agent 参数。

- 批量点击固定目标名单，每次写入前重新检查团队归属、profile、在线空闲状态和模式支持情况；跳过离线、运行中、已是目标模式或不支持目标模式的成员，单项失败不阻止剩余成员。关闭弹层或切换主会话后停止剩余操作；已发出的请求不回滚。
- 66 个文件、679 项常规测试，以及成员管理／集中审批／Teams 边界共 6 项 Chrome E2E 通过；类型检查、构建和严格消费方类型检查通过。新增批量行为测试覆盖正向修改、跨 profile 隔离、后来成员排除、部分失败和会话切换。浏览器验证运行中与休眠成员只读模式菜单、分组入口、刷新恢复、菜单关闭及审批布局；未把只读示例冒充在线空闲成员写入的浏览器验证。
- 原生 Teams 通常在成员任务完成后释放运行实例。此版本保留这一生命周期，不为了改模式唤醒或重建成员，也不写入待执行的模式变更。未调用真实模型，未提交或发布。

## 2026-09-14 — 休眠成员模式与发布候选 rc.2.4

- 主会话成员菜单可为休眠成员单独或按 ACP profile 批量保存模式。设置绑定原 ACP 会话和代际；恢复后、发送任务前重新检查并应用，Agent 确认后清除待应用记录。运行中及等待审批时不改权限，不额外唤醒 Agent。
- 同时覆盖允许／拒绝后休眠、刷新、批量保存、恢复后实际模式、失效模式与未确认响应不发送任务、绑定变化不继承设置。旧 SQLite 自动增加表，原有记录保留；延迟完成不能清除较新的设置。
- 两个 ACP 控制入口按当前模型路由显示；在安装 Teams profile 的原生模型会话中均隐藏，原生 Teams 入口保留。成员按钮直接复用输入栏样式，原生 Menu/StateDot 保持复用；中英文词条、参数与布局均核对，协议模式名称及审批原始说明不改写。
- 真实 Devin 目录增长至 385 个模型，暴露运行时配置快照截断到 256 个值的问题：目录中的 `gpt-5-4-mini-low` 位于第 340 项，导致切换被错误拒绝。现在保留有界的完整可选值集合，超过整体上限时明确失败，不截断后继续冒充完整能力。
- 默认回归使用四种确定性 ACP 协议夹具；真实冒烟另行运行。Claude 首次因父环境令牌不被继承返回 `ACP_AUTH_REQUIRED`，在隔离 profile 显式传入现有配置后通过；日常 profile 不变。

本轮验证：686 项单元／集成／契约测试通过；102 项确定性浏览器场景完成验证（最终全量中 101 项通过，成员管理用例因新页面尚未就绪而输入丢失；补齐页面等待后，该文件独立重跑通过）。成员模式的中英文、刷新、批量、恢复应用、原生会话隐藏和原生 Teams 保留均包含在该用例内；按钮样式与输入栏计算样式对照，保留演示额外检查不同卡片内容下的按钮底部对齐。

Claude Haiku（隔离 profile 显式凭据复测）、Codex Spark、Devin SWE-1.7 Medium、Kimi 的真实对话通过。修复目录截断后，Devin SWE-1.7 Medium → GPT-5.4 Mini Low 的真实 Teams 复测通过：两轮分别创建成员，成员与 Lead 完成有依赖的任务，消息回传，并完成真实成员审批验证。最终候选包通过严格公共类型消费、运行时依赖闭包、干净安装／移除、HTTP 200 与客户端 bootstrap 检查。没有触发 npm 发布。

- 成员模式显示补充回归：按钮与菜单勾选共同显示待生效模式，覆盖单独修改、批量修改、刷新及恢复后应用；保留“下次运行生效”提示。

## 2026-09-15：思考与回答分段修复

修复 ACP 主会话把非连续的同类内容写入同一个 DSH block index 的问题。只复用当前连续段的编号；内容类型或明确 messageId 变化时建立新段，工具首次出现／首次结束也在内容串行队列中建立边界，进度补丁与重复终态不切断 token。图片与非文本回退保持既有独立分段。

- 类型检查、构建及 693 项单元／集成／契约测试通过；新增 7 项边界测试覆盖交替、消息编号、可选编号、工具进度与终态、元数据及取消响应中的部分内容。
- 新增四种 ACP 协议的 Chrome E2E 全部通过：真实 Loader 和 ACP 子进程发送固定分段数据，验证同消息 token 正常拼接、思考／回答顺序、工具与消息边界、原生段落布局及刷新恢复。本轮未重跑全部 106 项 E2E，也未调用真实模型。
- 修复前后两份构建分别启动独立 DSH + Chrome，用完全相同的模拟输出生成折叠／展开截图，保存在 gitignored `.local/stream-comparison/`。旧实例保留错误行为供比较，新实例保留修复后的行为。未修改历史日志、未提交或发布。

### rc.2.5 发布回归

候选版本 `0.1.5-rc.2.5` 的正式 prepack 门禁（类型检查、693 项常规测试、构建、严格公共类型消费及打包闭包）全部通过；同一 tarball 通过已发布 DSH 的干净安装／移除、HTTP 200 和客户端 bootstrap 检查。
...
最终 prepack 通过：70 个测试文件、711 项常规测试，类型检查、构建、严格公共类型消费、tarball 闭包均通过。最终 tarball 通过精确 npm DSH 宿主的干净安装、移除、HTTP 200 和客户端 bootstrap。真实 Codex Luna、Devin SWE-1.7 Medium、Kimi Coding 两轮与刷新通过；Claude 当时因 OAuth 错误未完成验证；后续确认是隔离测试遗漏第三方服务的认证环境变量，补齐后通过，无需重新登录。配置方法见 [E2E 指南](README.md)。真实 Devin Teams 双模型、共享任务与依赖、成员消息和主会话审批通过。
