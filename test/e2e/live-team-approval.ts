...
export async function verifyLiveTeamApproval({ host, page, provider, evidence }: { host: AdapterWorld; page: Page; provider: string; evidence: Record<string, unknown> & { directory: string } }) {
  const token = `APPROVAL_${randomUUID().slice(0, 8)}`
  const fileName = `${token}.txt`
  const lead = required(host.ctx.agents.list().find(agent => agent.options.provider === provider && host.ctx.agentTeams.tryMembership(agent)?.role === 'lead'))
  const destination = join(required(lead.session.header.cwd), fileName)
  ...
  const member = required(host.ctx.agentTeams.listMembers(lead).find(member => member.name === 'permission-review'))
  evidence.memberId = member.id
  expect(existsSync(destination), 'Command must not run before approval').toBe(false)
  expect(events.filter(event => event.type === 'approval/asked').filter(event => event.sessionId === member.id)).toHaveLength(1)
  const handle = await host.ctx.sessionPersistence.open(member.id, 'read')
  try {
    const log = (await handle.read()).events
    const firstPrompt = log.find(event => event.type === 'user/message' && 'form' in event.data.source && event.data.source.form === 'snapshot')
