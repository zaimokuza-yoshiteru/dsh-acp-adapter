...
it.each(['allow', 'reject'])('handles eight member approvals from the Lead: %s, then leaves a later request pending', async decision => {
  ...
  try {
    await host.ctx.settings.replace('dsh-acp', { agents: { devin: {
      name: 'Fixture devin', command: process.execPath, args: [join(root, 'test/mock-agent/mock-agent.mjs')],
    ...
    expect(events.filter(event => event.type === 'approval/decided').at(-1).data.outcome).toBe('rejected')
