...
it('unloads and remounts ACP through the native plugin manager without restarting DSH', async () => {
  ...
  try {
    await host.ctx.settings.replace('dsh-acp', { agents: { devin: {
      name: 'Lifecycle fixture', command: process.execPath, args: [join(root, 'test/mock-agent/mock-agent.mjs')],
    ...
    const entry = (await manager.listPlugins()).find(row => row.moduleName === '@zaimokuza/dsh-acp-adapter')
