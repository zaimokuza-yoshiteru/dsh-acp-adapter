...
import { verifyLiveTeam } from './live-teams.ts'
import { verifyLiveTeamApproval } from './live-team-approval.ts'
import { verifyLiveSteering } from './live-steering.ts'
...
describe.skipIf(process.env.DSH_E2E_LIVE !== '1')('live ACP smoke', () => {
  it.each(profiles.filter(profile => selected === undefined || selected.includes(profile.id)))(teams ? '$id completes native shared tasks and member messaging' : steering ? '$id delivers steering after real generation begins' : '$id preserves host guidance and renders a real response', async profile => {
    ...
    let page!: Page
    let model!: string
    let settlement!: TurnEndReason
    const replies: { token: string; reply: string | undefined }[] = []
    const errors: string[] = []
    const started = Date.now()
    const teamEvidence: Record<string, unknown> & { secondModel?: string; secondRun?: Record<string, unknown>; approval?: Record<string, unknown> & { directory: string } } = {}
    ...
    try {
      ...
      } else browser = await launchBrowser(browserOptions)
      ...
      if (teams) {
        ...
        if (process.env.DSH_E2E_LIVE_TEAMS_MULTIMODEL === '1') {
          ...
          const second = required(models.find(entry => entry.id === 'gpt-5-4-mini-low'))
      ...
      for (let turn = 0; turn < 2; turn += 1) {
        ...
        try {
          ...
          settlement = required(log.findLast(event => event.type === 'turn/end')).data.reason
          expect(settlement).toMatchObject({ kind: 'completed' })
          const assistant = required(log.findLast(event => event.type === 'assistant/message'))
