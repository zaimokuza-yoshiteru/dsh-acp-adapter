...
class NativeTeamControl extends LlmAdapter {
  providerInfo(id: string) { return { id, name: 'Native team control' } }
  async listModels(provider: string) { return ['native-a', 'native-b'].map(id => ({ provider, id, name: id })) }
...
async function setup(teams: boolean, native = false) {
  ...
  const profile = (id: string) => ({ name: `Fixture ${id}`, command: process.execPath, args: [join(root, 'test/mock-agent/mock-agent.ts')], env: { MOCK_SCENARIO: 'regression', MOCK_PROFILE: id, MOCK_MCP_HTTP: '1' } })
  ...
  const browser = await launchBrowser({ channel: process.env.DSH_E2E_BROWSER_CHANNEL, headless: true })
...
it('keeps one ACP Agent across multiple models, isolates approvals and prevents cross-Agent teams', async () => {
  ...
  const evidence: Record<string, unknown> = {}, events: ObservedEvent[] = []
  ...
  try {
    ...
    const childAId = required(host.ctx.agentTeams.listMembers(lead).find(member => member.role === 'teammate')).id
    ...
    const childBId = required(host.ctx.agentTeams.listMembers(lead).find(member => member.name === 'calculator-b')).id
    expect(required(host.ctx.agents.get(childBId)).options).toMatchObject({ provider: 'acp-devin', model: 'mock-model-b' })
    expect(required(host.ctx.agents.get(childAId)).options).toMatchObject({ provider: 'acp-devin', model: 'mock-model-a' })
    ...
    const cardBox = required(await pendingCard.boundingBox())
    const composerBox = required(await page.locator('[data-composer-input]').first().boundingBox())
    ...
    const before = events.filter(event => event.type === 'request/header').filter(event => event.sessionId === childAId).length
    ...
    await vi.waitFor(() => expect(events.filter(event => event.type === 'request/header').filter(event => event.sessionId === childAId).length).toBeGreaterThan(before), { timeout: 30_000 })
    expect(required(events.filter(event => event.type === 'request/header').findLast(event => event.sessionId === childAId)).data.header.config).toMatchObject({ provider: 'acp-devin', model: 'mock-model-a' })
    await page.getByRole('button', { name: /^Select model/ }).click()
    await page.getByRole('menuitem', { name: /^Model/ }).click()
    ...
    const destinationId = required(events.findLast(event => event.type === 'request/header' && event.data.header.config.provider === 'acp-codex')).sessionId
    expect(destinationId).not.toBe(lead.id)
    expect(host.ctx.agentTeams.listMembers(required(host.ctx.agents.get(destinationId)))).toHaveLength(1)
    ...
    await host.ctx.sessionController.prompt({ requestId: 'cross-agent-negative' as SessionRequestId, sessionId: lead.id, mode: 'queue', content: [{ type: 'text', text: 'E2E_MESSAGE' }] }, new AbortController().signal)
...
it('preserves native Teams model inheritance and switching with the ACP plugin installed', async () => {
  ...
  const headers: { id: string; config: { provider: string; model: string } }[] = [], settled: { id: string; reason: unknown }[] = []
  ...
  try {
    ...
    const spawn = (name: string) => host.ctx.agentTeams.spawnTeammate(required(lead), { name, description: name, prompt: [{ type: 'text', text: 'Calculate 1+1' }], context: 'fresh', provider: 'spawn', signal: new AbortController().signal })
    ...
    expect(required(host.ctx.tools.get('spawn_teammate', lead)).parameters).toMatchObject({ properties: { context: { enum: expect.arrayContaining(['fork']) } } })
  } finally { await browser.close(); await host.close() }
}, 90_000)
