...
export async function verifyLiveSteering({ host, page }: { host: AdapterWorld; page: Page }) {
  ...
  try {
    ...
    try {
      ...
      expect(required(events.findLast(event => event.type === 'turn/end')).data.reason).toMatchObject({ kind: 'completed' })
      ...
      const assistant = required(events.findLast(event => event.type === 'assistant/message'))
