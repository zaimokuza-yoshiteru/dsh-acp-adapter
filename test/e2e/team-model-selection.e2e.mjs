...
it('persists teammate model selection, applies it on wake, and protects Team boundaries', async () => {
  ...
  try {
    ...
    const child = host.ctx.agentTeams.listMembers(lead).find(member => member.role === 'teammate')
    ...
    await expect(host.ctx.dshAcp.setTeamMemberModel(lead.id, child.id, 'mock-model-b')).rejects.toThrow()
    await expect(host.ctx.dshAcp.teamMemberModels('foreign-lead', child.id)).rejects.toThrow()
    await expect(host.ctx.dshAcp.teamMemberModels(lead.id, lead.id)).rejects.toThrow()
    ...
    await expect(host.ctx.dshAcp.setTeamMemberModel(lead.id, child.id, 'unknown-model')).rejects.toThrow()
    const beforeSaveLog = readFileSync(log, 'utf8')
    const saved = await host.ctx.dshAcp.setTeamMemberModel(lead.id, child.id, 'mock-model-b')
    ...
    const initial = await host.ctx.dshAcp.teamMemberModels(lead.id, child.id)
    ...
    await expect.poll(() => host.ctx.dshAcp.teamMemberModels(lead.id, child.id), { timeout: 30000 }).toMatchObject({ currentModel: 'mock-model-b', pendingModel: null, writable: true })
    ...
    expect(host.ctx.get('agentDefaultModel').currentSelection()).toMatchObject({ provider: 'acp-devin', model: 'mock-model-a' })
    expect(lead.session.requestHeader()?.config.model).toBe('mock-model-a')
    expect(events.filter(event => event.sessionId === child.id && event.type === 'request/header').slice(-2).map(event => event.data.header.config.model)).toEqual(['mock-model-b', 'mock-model-b'])
    await send('E2E_TEAM_SECOND')
    await page.getByText('E2E_TEAM_SECOND_READY', { exact: true }).waitFor()
    const sibling = host.ctx.agentTeams.listMembers(lead).find(member => member.name === 'calculator-b')
    await expect.poll(() => events.findLast(event => event.sessionId === sibling.id && event.type === 'request/header')?.data.header.config.model).toBe('mock-model-a')
    expect((await host.ctx.dshAcp.teamMemberModels(lead.id, child.id)).currentModel).toBe('mock-model-b')
  } finally {
    await browser?.close()
    await host.close()
  }
