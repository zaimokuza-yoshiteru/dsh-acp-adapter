...
export async function hostToolsTurn(session, msg, { sendUpdate, sendAgentRequest, respond }) {
  ...
  if (!present && !prompt.includes('E2E_HOST_TOOLS')) return false
  ...
  try {
    ...
    const tool = tools.find(tool => tool.name.endsWith(present ? '_present' : prompt.includes('E2E_HOST_TOOLS_LATE') ? '_e2e_late_fixture' : '_e2e_fixture'))
    ...
    const result = await client.callTool({ name: tool.name, ...(present ? { arguments: { files: [{ path: 'delivery.txt', description: 'ACP delivery through native DSH' }] } } : {}) })
    if (result.isError || !JSON.stringify(result).includes(present ? 'Presented delivery.txt' : 'E2E_HOST_POST')) throw new Error(`Native hook output missing: ${JSON.stringify(result)}`)
