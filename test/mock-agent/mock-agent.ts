...
const state = {
  ...
  sessions: new Map<string, MockSession>(), // sessionId -> { id, cwd, modes, configOptions, turn }
  sessionSeq: 0,
  agentReqSeq: 0,
...
function freshConfigOptions() {
  ...
  const options: MockConfigOption[] = [
    {
      id: 'mode',
      name: 'Session Mode',
      category: 'mode',
...
async function runElicitationTurn(session: MockSession, msg: PromptMessage) {
  const turn: MockTurn = {
    cancelled: false,
    cancel() {
      this.cancelled = true;
...
function runCancelStuckTurn(session: MockSession, _msg: PromptMessage) {
  const turn: MockTurn = {
    cancelled: false,
    cancel() {
      this.cancelled = true;
