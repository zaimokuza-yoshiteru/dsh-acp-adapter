 ...
 * their stable ids also occur in the request projection.
 */
...
export interface CurrentStepProof {
  ...
  /** Whether the request projection contained anything other than the direct
   * user input admitted for this step. This is a bounded diagnostic fact; the
  ...
  constructor(readonly code:
    ...
    super(code === 'ACP_SESSION_UNAVAILABLE'
        ...
        : code === 'ACP_NO_OPEN_STEP'
          ...
          : 'the current DSH AgentLoop step contains no direct user input')
...
export function admitCurrentStep(
  ...
  const admitted = currentIds
    ...
    .filter((message): message is Message => message !== undefined && message.role === 'user' && message.source.kind === 'user')
