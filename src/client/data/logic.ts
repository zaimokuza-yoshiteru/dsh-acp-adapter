...
/**
 * The honest login state ACP v1 can report: the protocol has no
 * "am I authenticated" query, so a probed agent exposes only whether it
 * ADVERTISES auth methods; `unknown` covers never-probed, probe-error, and
 * hosts that do not pass authMethods through.
 */
export type LoginState =
  | { kind: 'none' }
  | { kind: 'methods'; methods: readonly AcpAuthMethod[] }
  | { kind: 'unknown' }

/** Derive one row's login state (undefined row = the health response did not cover this agent). */
export function loginStateOf(row: AcpProviderHealth | undefined): LoginState {
  if (row === undefined) return { kind: 'unknown' }
  if (row.probe.status !== 'ok') return { kind: 'unknown' }
  const methods = row.probe.authMethods
  if (methods === null) return { kind: 'unknown' }
  if (methods.length === 0) return { kind: 'none' }
  return { kind: 'methods', methods }
}

/** Whether the row's last probe failed for lack of login (: show the loginHint there). */
export function probeNeedsLogin(row: AcpProviderHealth | undefined): boolean {
  return row !== undefined && row.probe.status === 'error' && row.probe.failureKind === ACP_FAILURE_AUTH_REQUIRED
}

/** Whether the row needs a login nudge（probe auth_required 或 五态 auth-required）。 */
export function rowNeedsLogin(row: AcpProviderHealth | undefined): boolean {
  return probeNeedsLogin(row) || row?.state === 'auth-required'
}

/** Whether the card's login guidance line should show (login demanded, or methods advertised). */
export function showsLoginHint(row: AcpProviderHealth | undefined): boolean {
  return rowNeedsLogin(row) || loginStateOf(row).kind === 'methods'
}
...
/**
 * 一层健康的状态词表：
 * - `ok`：该层事实已通过（executable=文件在；initialize=握手成功；session=session/new 成功）
 * - `failed`：该层探测失败（failureKind 分流）
 * - `blocked`：被上层失败阻断，本层未到达
 * - `unknown`：未探测/数据缺席
 * - `unverified`：无零消耗探测手段，诚实标「未验证」（prompt-auth 层的常态——
 *   ACP 没有「是否已登录」查询，initialize/session 通过不代表 prompt 级鉴权可用）
 * - `needsLogin`：probe 被 auth_required 拒绝，需要先登录
 */
export type HealthLayerState = 'ok' | 'failed' | 'blocked' | 'unknown' | 'unverified' | 'needsLogin'

/** 一层的展示事实。 */
export interface HealthLayerView {
  state: HealthLayerState
  /** failed 层的 probe 失败分流 kind（AcpErrorKind 透传）。 */
  failureKind?: string
  /** session 层 ok 时的模型选项数（0 = agent 未广告模型选项，如实显示）。 */
  modelCount?: number
}

/** 健康卡的四层视图（顺序即分层序）。 */
export interface HealthLayers {
  executable: HealthLayerView
  initialize: HealthLayerView
  session: HealthLayerView
  promptAuth: HealthLayerView
}

/**
 * 从一行健康数据推导四层视图。分层语义（与 src/remote/service.ts 的 probe
 * 阶段标记对齐）：probe 到达 `session` 阶段即证明 initialize 已通过；phase
 * 缺席（null，未标记的失败）时 initialize 记 failed、session 记 unknown
 * （不猜测到达点）。行缺席（端点未覆盖该 agent）四层全 unknown。
 */
export function healthLayersOf(row: AcpProviderHealth | undefined): HealthLayers {
  if (row === undefined) {
    return {
  ...
  const executable: HealthLayerView = row.executable ? { state: 'ok' } : { state: 'failed' }
  const probe = row.probe
  if (probe.status === 'never') {
    return {
      executable,
      initialize: { state: 'unknown' },
      session: { state: 'unknown' },
  ...
  if (probe.status === 'ok') {
    return {
      executable,
      initialize: { state: 'ok' },
      session: { state: 'ok', modelCount: probe.modelCount },
      promptAuth: { state: 'unverified' },
    }
  }
  // error 分支：按 phase 分层；auth_required 的 prompt-auth 层给可行动状态
  const failure: HealthLayerView = { state: 'failed', failureKind: probe.failureKind }
  const promptAuth: HealthLayerView = probe.failureKind === ACP_FAILURE_AUTH_REQUIRED
    ? { state: 'needsLogin' }
    : { state: 'blocked' }
  if (probe.phase === 'session') {
    return { executable, initialize: { state: 'ok' }, session: failure, promptAuth }
  }
  if (probe.phase === 'initialize') {
    return { executable, initialize: failure, session: { state: 'blocked' }, promptAuth }
  }
  // 阶段未标记：不猜测到达点
  return { executable, initialize: failure, session: { state: 'unknown' }, promptAuth }
}
