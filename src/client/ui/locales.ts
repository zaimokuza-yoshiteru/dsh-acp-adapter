...
const zh = {
  ...
  stateReady: '协议可用',
  subagentProjectionTitle: '将外部委派加入 DSH 子代理目录',
  subagentProjectionDescription: '默认关闭。启用后，DSH 会用原生只读会话展示 Agent 已提供的委派任务与最终输出或摘要；不会补造未暴露的内部轨迹，也不能继续对话。',
...
const en: Record<AcpLocaleKey, string> = {
  ...
  subagentProjectionTitle: 'Add external delegations to the DSH subagent catalog',
  subagentProjectionDescription: 'Off by default. DSH uses a native read-only transcript for the task and final output or summary exposed by the Agent. Unexposed internal traces are not invented, and the session cannot be continued.',
